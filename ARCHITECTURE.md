...
**The six that were real gaps are now wired.**
...
- **`secrets[].channels`** decides how a secret is found — and for `notes`, `strange-objects`, `serial-numbers` and `diagnostics`, `defaultConditions` infers `{ kind: 'inspecting', targetId: secret.id }`. Twenty-eight of the catalogue's forty-seven carry one of those channels, and until `packages/sim/src/curios.ts` existed **no code anywhere in the product wrote `presence.inspecting`**: the condition was unsatisfiable, so the secrets were authored, validated, shipped and unreachable. The gap was spatial rather than logical. A condition that says *you are inspecting this secret* only means something if the secret is a thing standing somewhere, and a secret has an id, a title and two pieces of prose but no position. `placeCurios` gives it one — from the campsite's own seed, at a fraction of the walkable radius, out of the fire and clear of the landmarks, deterministic across devices and visits — and nothing is authored for it: the label is the secret's `title`, which was already written to be read by a player, and the shape is read from the nouns in that title (`SHAPE_WORDS`) with the channel as a fallback. `stepDiscovery` then treats a deliberate secret differently from a witnessed one: five to twelve seconds of a held look and no rarity roll, against twenty-six seconds and a roll for something you have to be there for. Crouching over a thing is not luck.
