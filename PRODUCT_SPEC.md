...
**And it is a different night every night.** Each environment names five things about itself that change between visits, with a range and a unit for each: how much wood is on the stack, how wet the duff is, whether the marine layer is in, how high the creek is, how far off the storm is, how bold the grey jay is tonight. They are rolled from the campsite *and which visit this is* — so going back finds the same place on a different night, a shared night is the same night for everybody at that fire, and a replay is the same night again. Each draws from its own named stream, so adding one to a campsite cannot change the others. What stays fixed between visits is stated separately, as the campsite's invariants: the site post and its violet card, the dent in the bear box, the loose middle stepping stone. The roll never rewrites the place: a salt flat whose manifest gives fog a weight of zero does not fog, however hazy the night rolls, and an animal is never made so shy it cannot be found. About a fifth of what is declared drives nothing yet, and is recorded as driving nothing rather than quietly ignored.

**And its horizon is as closed as it says it is.** How much canopy a campsite has is an axis the catalogue grades every environment on, from a salt pan with none to a temperate rainforest with the sky in shreds. The treeline is drawn from that axis; the vegetation kits decide what grows underneath and where inside its band a particular wood sits.

**The survey says what a campsite is for.** Every environment marks one activity as the thing it exists to offer — the tide pools at the north end, the slot that answers you back at two delays, the box you can sit in with steam coming off it. A player who can see the screen finds those by walking into them. Asking what is around you names it, because otherwise that player finds it by accident or not at all.
...
- Every act in the ritual is the act, not a control that performs it. Taking the
  marshmallow off the fire is pulling the stick back past the coals; taking the
  sandwich out is taking hold of it and lifting. Where a button remains for the
  alternate control scheme it is in the accessibility tree and not on the
  screen, so neither path is a degraded version of the other.
