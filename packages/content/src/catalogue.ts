...
import type { EnvironmentManifest, RegionId } from './schema.js';

const BY_ID: ReadonlyMap<string, EnvironmentManifest> = new Map(
  ENVIRONMENTS.map((environment) => [environment.id, environment]),
);

/** Every launch environment, in authoring order. */
export function listEnvironments(): readonly EnvironmentManifest[] {
  return ENVIRONMENTS;
}
...
export function getEnvironment(id: string): EnvironmentManifest | undefined {
  return BY_ID.get(id);
}
 ...
 */
export function requireEnvironment(id: string): EnvironmentManifest {
 ...
 */
export function environmentsByRegionalAffinity(
  region: RegionId,
  weightOf: (environment: EnvironmentManifest, region: RegionId) => number,
