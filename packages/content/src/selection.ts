...
export interface SelectionOptions {
  /** Anything stable per player/campsite. Same seed ⇒ same draw, always. */
  readonly seed: number | string;
  /** Approximate region, if the player granted it. Defaults to `unknown`. */
  readonly region?: RegionId;
  /** Environments the player has already found — deprioritised, never banned. */
  readonly discoveredIds?: readonly string[];
  /** Defaults to the launch catalogue. Present so live ops and tests can pass their own. */
  readonly catalogue?: readonly EnvironmentManifest[];
}
 ...
 */
export function effectiveDiscoveryWeight(
 ...
 */
export function selectEnvironment(options: SelectionOptions): EnvironmentManifest {
  ...
  return picked ?? (pool[0] as EnvironmentManifest);
}
 ...
 */
export function discoveryOrder(options: SelectionOptions): readonly EnvironmentManifest[] {
  const catalogue = options.catalogue ?? ENVIRONMENTS;
  const region = options.region ?? 'unknown';
  const rng = new Rng(options.seed).split(DISCOVERY_STREAM);

  const remaining = [...catalogue];
  const order: EnvironmentManifest[] = [];

  while (remaining.length > 0) {
    const picked = rng.weightedPick(remaining, (environment) =>
      effectiveDiscoveryWeight(environment, region),
    );
    const chosen = picked ?? (remaining[0] as EnvironmentManifest);
    order.push(chosen);
 ...
 */
export function unreachableEnvironments(
  region: RegionId,
  catalogue: readonly EnvironmentManifest[] = ENVIRONMENTS,
