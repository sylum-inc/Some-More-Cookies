...
import { ALL_ENVIRONMENTS, MAX_REGION_AFFINITY, MIN_REGION_AFFINITY, REGIONS, REVERB_SPACES, SEASONAL_EVENT_KINDS, } from './schema.js';
class Issues {
    list = [];
    add(path, message) {
...
export const WEATHER_KINDS = keysOf({
    clear: true,
    'high-cloud': true,
    overcast: true,
    'light-rain': true,
...
export const SKY_EVENTS = keysOf({
    'meteor-shower': true,
    'heat-lightning': true,
    aurora: true,
    moonbow: true,
...
const QUIRK_IDS = QUIRK_POOL.map((q) => q.id);
/* -------------------------------------------------------------------------- */
...
function str(issues, path, value, maxLength = 4000) {
...
function num(issues, path, value, min, max) {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        issues.add(path, 'must be a finite number');
        return null;
    }
    if (value < min || value > max)
        issues.add(path, `must be within [${min}, ${max}] (got ${value})`);
    return value;
}
function unit(issues, path, value) {
    return num(issues, path, value, 0, 1);
}
function oneOf(issues, path, value, allowed) {
    if (typeof value !== 'string' || !allowed.includes(value)) {
        issues.add(path, `must be one of: ${allowed.join(', ')}`);
    }
}
function hex(issues, path, value) {
    if (typeof value !== 'string' || !HEX_PATTERN.test(value)) {
        issues.add(path, 'must be a #rrggbb colour');
    }
}
function range(issues, path, value, min = -1e9, max = 1e9) {
    if (!isRecord(value)) {
    ...
    if (lo !== null && hi !== null && lo > hi)
        issues.add(path, `min (${lo}) must not exceed max (${hi})`);
}
function arrayOf(issues, path, value, minLength, maxLength = 512) {
    if (!Array.isArray(value)) {
        issues.add(path, 'must be an array');
        return null;
    }
    if (value.length < minLength)
        issues.add(path, `must contain at least ${minLength} item(s)`);
    if (value.length > maxLength)
        issues.add(path, `must contain at most ${maxLength} items`);
    return value;
}
function stringsFrom(issues, path, value, allowed, minLength) {
    const items = arrayOf(issues, path, value, minLength);
    if (!items)
        return;
    items.forEach((item, i) => oneOf(issues, `${path}[${i}]`, item, allowed));
}
function prose(issues, path, value, keys) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
    for (const key of keys)
        str(issues, `${path}.${key}`, value[key]);
}
/* -------------------------------------------------------------------------- */
/* Section validators                                                         */
/* -------------------------------------------------------------------------- */
function validateCharacter(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
...
function validateArrival(issues, path, value) {
...
function validateScene(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
    ...
    kits?.forEach((kit, i) => {
        ...
        if (kitId !== null && !KEY_PATTERN.test(kitId))
            issues.add(`${kitPath}.kitId`, 'must match ^[a-z0-9_]+$');
        ...
        if (typeof kit['lowTierDrop'] !== 'boolean')
            issues.add(`${kitPath}.lowTierDrop`, 'must be a boolean');
    ...
    landmarks?.forEach((landmark, i) => {
        ...
        if (lmId !== null && !KEY_PATTERN.test(lmId))
            issues.add(`${lmPath}.id`, 'must match ^[a-z0-9_]+$');
        str(issues, `${lmPath}.label`, landmark['label']);
        oneOf(issues, `${lmPath}.kind`, landmark['kind'], LANDMARK_KINDS);
        if (typeof landmark['handcrafted'] !== 'boolean')
            issues.add(`${lmPath}.handcrafted`, 'must be a boolean');
    ...
    if (water !== undefined) {
        ...
        if (!isRecord(water)) {
            ...
            if (water['kind'] === 'none')
                issues.add(`${wPath}.kind`, 'omit the water feature entirely instead of using "none"');
            ...
            if (typeof water['fishable'] !== 'boolean')
                issues.add(`${wPath}.fishable`, 'must be a boolean');
            if (typeof water['skippable'] !== 'boolean')
                issues.add(`${wPath}.skippable`, 'must be a boolean');
    ...
    if (!isRecord(palette)) {
        ...
        if (water2 !== null)
            hex(issues, `${path}.nightPalette.water`, water2);
    }
}
function validateWeather(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
    ...
    if (!isRecord(weights)) {
        ...
        if (entries.length === 0)
            issues.add(`${path}.weights`, 'must weight at least one weather kind');
    ...
    events?.forEach((event, i) => {
        ...
        if (typeof event !== 'string' || !SKY_EVENTS.includes(event)) {
...
function validateFuel(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
    ...
    sources?.forEach((source, i) => {
        ...
        if (typeof woodId !== 'string' || WOOD_TYPES[woodId] === undefined) {
            issues.add(`${sPath}.woodId`, `must be a WOOD_TYPES id (one of: ${Object.keys(WOOD_TYPES).join(', ')})`);
        }
...
function validateWildlife(issues, path, value) {
    ...
    roster.forEach((entry, i) => {
        ...
        if (id !== null) {
            if (!KEY_PATTERN.test(id))
                issues.add(`${ePath}.id`, 'must match ^[a-z0-9_]+$');
            if (seen.has(id))
                issues.add(`${ePath}.id`, `duplicate wildlife id "${id}"`);
            seen.add(id);
        }
        ...
        if (typeof entry['canPersist'] !== 'boolean')
            issues.add(`${ePath}.canPersist`, 'must be a boolean');
...
function validateAmbience(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
    ...
    events?.forEach((event, i) => {
        ...
        if (id !== null) {
            if (!KEY_PATTERN.test(id))
                issues.add(`${ePath}.id`, 'must match ^[a-z0-9_]+$');
            if (seen.has(id))
                issues.add(`${ePath}.id`, `duplicate distant sound id "${id}"`);
            seen.add(id);
        }
...
function validateActivities(issues, path, value) {
    ...
    list.forEach((entry, i) => {
        ...
        if (typeof id === 'string') {
            if (seen.has(id))
                issues.add(`${ePath}.id`, `duplicate activity "${id}"`);
            seen.add(id);
        }
        str(issues, `${ePath}.label`, entry['label']);
...
function validateRadio(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
    ...
    stations?.forEach((station, i) => {
        ...
        if (id !== null) {
            if (!KEY_PATTERN.test(id))
                issues.add(`${sPath}.id`, 'must match ^[a-z0-9_]+$');
            if (seen.has(id))
                issues.add(`${sPath}.id`, `duplicate station id "${id}"`);
            seen.add(id);
        }
...
function validateSecrets(issues, path, value) {
    ...
    list.forEach((entry, i) => {
        ...
        if (id !== null) {
            if (!KEY_PATTERN.test(id))
                issues.add(`${ePath}.id`, 'must match ^[a-z0-9_]+$');
            if (seen.has(id))
                issues.add(`${ePath}.id`, `duplicate secret id "${id}"`);
            seen.add(id);
        }
        ...
        if (entry['gatesNothing'] !== true) {
            issues.add(`${ePath}.gatesNothing`, 'must be literally true — a discovery may never gate essential functionality or a major reward');
        }
        ...
        if (oneTime) {
            if (typeof evidence !== 'string' || evidence.trim().length === 0) {
                issues.add(`${ePath}.leavesEvidence`, 'a one-time discovery must leave evidence afterward — a missed event never strands anyone (spec §8)');
            }
        }
...
function validateMachine(issues, path, value) {
    ...
    const entries = Object.entries(weights);
    if (entries.length === 0)
        issues.add(`${path}.quirkWeights`, 'must weight at least one quirk');
...
function validateProcedural(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
    ...
    streams?.forEach((stream, i) => {
        ...
        if (name !== null && !KEY_PATTERN.test(name))
            issues.add(sPath, 'must match ^[a-z0-9_]+$');
    });
    ...
    variations?.forEach((variation, i) => {
        ...
        if (id !== null) {
            if (!KEY_PATTERN.test(id))
                issues.add(`${vPath}.id`, 'must match ^[a-z0-9_]+$');
            if (seen.has(id))
                issues.add(`${vPath}.id`, `duplicate variation id "${id}"`);
            seen.add(id);
        }
...
function validateDiscovery(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
    ...
    if (typeof weight !== 'number' || !Number.isFinite(weight) || weight <= 0) {
        issues.add(`${path}.weight`, 'must be a finite weight greater than zero — every core environment must remain discoverable (spec §5.4)');
    }
    str(issues, `${path}.note`, value['note']);
    ...
    for (const [region, multiplier] of Object.entries(affinities)) {
        ...
        if (multiplier < MIN_REGION_AFFINITY || multiplier > MAX_REGION_AFFINITY) {
            issues.add(aPath, `must be within [${MIN_REGION_AFFINITY}, ${MAX_REGION_AFFINITY}] — region weights early appearance and may never lock an environment out`);
        }
    }
}
function validatePerformance(issues, path, value) {
    if (!isRecord(value)) {
        issues.add(path, 'must be an object');
        return;
    }
...
export function validateEnvironment(value, pathPrefix) {
    ...
    tags?.forEach((tag, i) => {
        ...
        if (text !== null && !/^[a-z0-9-]+$/.test(text))
            issues.add(tPath, 'must match ^[a-z0-9-]+$');
    });
    ...
    if (isRecord(character) && isRecord(scene)) {
        ...
        if (isRecord(feature) && declared !== 'none' && feature['kind'] !== declared) {
            issues.add(`${path}.scene.water.kind`, `must match character.water ("${String(declared)}"), got "${String(feature['kind'])}"`);
        }
        if (isRecord(scene['nightPalette'])) {
            const paletteWater = scene['nightPalette']['water'];
...
export function validateCatalogue(value) {
    ...
    const seen = new Set();
    list.forEach((entry, i) => {
        for (const issue of validateEnvironment(entry, `catalogue[${i}]`))
            issues.list.push(issue);
        if (isRecord(entry) && typeof entry['id'] === 'string') {
            const id = entry['id'];
            if (seen.has(id))
                issues.add(`catalogue[${i}].id`, `duplicate environment id "${id}"`);
            seen.add(id);
        }
    });
    return issues.list;
...
export function assertValidCatalogue(value) {
...
export function assertValidEnvironment(value) {
...
function validateEnvironmentTargets(issues, path, value) {
    ...
    list.forEach((entry, i) => {
        ...
        if (seen.has(id))
            issues.add(ePath, `duplicate environment target "${id}"`);
        seen.add(id);
    });
...
function validateStationList(issues, path, value, minLength, maxLength) {
 ...
 */
export function validateSeasonalEvent(value, pathPrefix) {
    const issues = new Issues();
    if (!isRecord(value)) {
    ...
    rewardCodes?.forEach((code, i) => {
        ...
        if (text !== null && !KEY_PATTERN.test(text))
            issues.add(cPath, 'must match ^[a-z0-9_]+$');
    });
    ...
    if (value['exclusive'] !== undefined || value['gates'] !== undefined) {
        issues.add(`${path}.exclusive`, 'seasonal content may never gate anything; a missed window must strand nobody (spec §5.5, §8)');
    }
    return issues.list;
}
/** A block of radio programming pushed at one or more environments. */
export function validateStationProgramming(value, pathPrefix) {
    const issues = new Issues();
    if (!isRecord(value)) {
...
export function assertValidSeasonalEvent(value) {
...
/** Throws with every problem listed. Mirrors `assertValidEnvironment`. */
export function assertValidStationProgramming(value) {
